...
locals {
  distribution_for_main_website__origin_id = "${var.service_name_hyphens}--${var.environment_hyphens}--Main-Website-origin"
}
...
resource "aws_cloudfront_distribution" "distribution__main_website" {
  ...
  origin {
    ...
    origin_id = local.distribution_for_main_website__origin_id
  ...
  default_cache_behavior {
    ...
    target_origin_id = local.distribution_for_main_website__origin_id
    viewer_protocol_policy = "redirect-to-https"
    compress = true

    function_association {
      event_type   = "viewer-request"
      function_arn = (var.environment == "Prod"
                      ? aws_cloudfront_function.index_html_function[0].arn
                      : aws_cloudfront_function.http_basic_auth_function[0].arn)
    }
  }
...
resource "aws_cloudfront_function" "index_html_function" {
  ...
  name    = "${var.service_name_hyphens}--${var.environment_hyphens}--index-html-function"
